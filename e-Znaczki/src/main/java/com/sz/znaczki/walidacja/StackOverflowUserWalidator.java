...
public class StackOverflowUserWalidator implements Walidator<String> {

	static final Logger LOGGER = LogManager.getLogger(StackOverflowUserWalidator.class);
	public static final String STACK_OVERFLOW_USER_URL = "https://api.stackexchange.com/2.2/users/{id}?order=desc&sort=reputation&site=stackoverflow";

	private static Walidator<String> porpawneId = str -> str.length() > 0 && str.matches("(\\d)*");
